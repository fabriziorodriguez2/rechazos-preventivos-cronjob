...
ONAJPU_BROKER_ID = 956

# ============================================================
# Nomenclatura campaña ONAJPU
# ============================================================
...
def _construir_nombre_campana(periodo):
    """SC-ONAJPU- L[YYMM]"""
    yymm = periodo[2:]
    return f"SC-ONAJPU- L{yymm}"


def _construir_codigo(periodo):
    """ONAJPU-[YYMM]"""
    yymm = periodo[2:]
    return f"ONAJPU-{yymm}"
...
def resolver_contacto(conn, row):
    ...
    Retorna dict { id_contacto, id_tel_fijo1 } o None si no encuentra.
    Prioridad: id_servicio > cedula.
    """
    id_servicio = row.get("id_servicio")
    cedula = row.get("cedula")

    # Prioridad 1: id_servicio
    if id_servicio:
        ...
        # id_servicio dado pero no válido – no caer a cedula (dato explícito)
        return None
...
# ============================================================
# Campaña mensual ONAJPU (crear o reutilizar)
# ============================================================

def obtener_o_crear_campana(conn, periodo):
    """
    Busca la campaña ONAJPU del mes. Si no existe, la crea.
    Retorna campaign_id.
    """
    ...
    if row:
        logger.debug("Campaña ONAJPU existente: id=%s codigo=%s", row["id"], codigo)
        return row["id"]

    # Fallback: nombre LIKE (campañas creadas manualmente)
    yymm = periodo[2:]
    row = execute_one(
        conn,
    ...
    if row:
        logger.info("Campaña ONAJPU encontrada por nombre (formato previo): id=%s", row["id"])
        return row["id"]

    # Crear nueva
    brokers = f"_{ONAJPU_BROKER_ID}_"
    descripcion = (
        f"Camp. ONAJPU {periodo}. Generado automáticamente."
    )
    ...
    execute_query(
        ...
        (campaign_id, ONAJPU_BROKER_ID),
    )
    commit(conn)

    logger.info("Campaña ONAJPU creada: id=%s codigo=%s nombre=%s", campaign_id, codigo, nombre)
    return campaign_id
...
def insertar_gestion(conn, campaign_id, id_contacto, id_tel_fijo1):
    """
    Inserta una gestión y retorna su id.
    """
    ts = _ts()
    execute_query(
        conn,
        ...
        """,
        (campaign_id, ONAJPU_BROKER_ID, id_contacto, ts, id_tel_fijo1),
    )
    commit(conn)
    row = execute_one(conn, "SELECT LAST_INSERT_ID() AS id")
    return row["id"]
...
def procesar_fila(conn, staging_row, campaign_id):
    """
    Procesa una fila de onajpu_staging.
    Retorna True si fue exitosa, False si hubo error controlado.
    """
